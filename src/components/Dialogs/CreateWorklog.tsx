...
const CreateWorkLogDialog: FC<Props> = (props) => {
  ...
  const [workLogActualStartTime, setWorkLogActualStartTime] = useState<
    Date | undefined
  >(new Date());
  const [workLogActualEndTime, setWorkLogActualEndTime] = useState<
    Date | undefined
  >(new Date());
  ...
  const initData = () => {
    if (!props.isEdit) {
      ...
      setWorkLogActualEndTime(
        new Date(Number.parseInt(props?.worklog?.end_time?.value))
      );
      setWorkLogActualStartTime(
        new Date(Number.parseInt(props?.worklog?.start_time?.value))
      );
  ...
  const handlingCreateWorkLog = () => {
    const worklog: IWorklogRoot = {
      worklog: {
        ...
        worklog_type: {
          id:
            types?.filter((type: any) => type?.name === selectedType)?.[0]
              ?.id ?? "",
        },
      },
    };
    ...
    <React.Fragment>
      <Box>
        <Typography level="title-sm" sx={{ fontWeight: "bold" }}>
          Project : {appDataContext?.project?.title ?? ""}
        </Typography>
        <Typography level="body-sm" sx={{ fontWeight: "bold" }}>
          Task ID: : {appDataContext?.task?.id ?? ""}
        </Typography>
        ...
        <Box sx={{ padding: 1 }}>
            ...
            >
              <FormControl>
                ...
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                  <DateTimePicker
                    onChange={(value) => setWorkLogActualStartTime(value)}
                    value={workLogActualStartTime}
                    ampm={false}
                    sx={{ width: 250 }}
              ...
              <FormControl>
                ...
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                  <DateTimePicker
                    onChange={(value) => setWorkLogActualEndTime(value)}
                    value={workLogActualEndTime}
                    ampm={false}
                    sx={{ width: 250 }}
                    slotProps={{ textField: { size: "small" } }}
                  />
          ...
          >
            ...
            <FormControl sx={{ width: 200 }}>
              <FormLabel>Worklog Type</FormLabel>
              <Select
                value={selectedType}
...
const mapDispatchToProps = (dispatch: any) => {
  return {
    ...
    onAddWOrkLog: (projectId: string, taskId: string, payload: IWorklogRoot) =>
      dispatch(
        addWorklog({
          projectId: projectId,
          taskId: taskId,
    ...
    onGetTypes: (projectId: string, taskId: string) =>
      dispatch(
        getTypes({
          projectId: projectId,
          taskId: taskId,
